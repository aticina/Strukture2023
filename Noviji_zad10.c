...
position_tree createTreeRoot();
position_list createHeadList();
int insertIntoList(position_list, position_list);
position_list createListElement(char [], int);
int printInorder(position_tree);
position_tree creteTreeElement(char []);
position_tree insertIntoTree(position_tree, position_tree);
position_tree readFromFile(char[]);
int readFromAdditionalFile(position_tree, char[]);
int printList(position_list);
position_tree findStateByName(position_tree, char[]);
position_list findCityByName(position_list, char[]);
int Menu(position_tree);
...
position_tree readFromFile(char fileName[]) {
    ...
    FILE* fp = NULL;
    char* buffer = NULL;
    char stateName[MAX_CHAR] = { 0 };
    char additionalFile[MAX_CHAR] = { 0 };
    int byteNumb = 0;
    position_tree newElement = NULL;
    position_tree root = NULL;

    buffer = (char*)malloc(MAX_CHAR * sizeof(char));
